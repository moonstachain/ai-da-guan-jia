...
CORE_PROFILES: dict[str, SkillProfile] = {
    ...
    "feishu-km": SkillProfile(
        name="feishu-km",
        role="Operate Feishu Aily or ask.feishu knowledge Q&A with raw answer preservation and scope-aware verification.",
        strengths=["Official Aily API path", "ask.feishu alignment", "Permission triage", "Raw Q&A preservation"],
        weaknesses=["Depends on Feishu app credentials or a manual web workflow during transition"],
        boundary="Use before downstream synthesis when the user wants Feishu knowledge Q&A rather than Wiki reading or generic Feishu docs extraction.",
        keywords=[
            "feishu-km",
            "ask.feishu",
            "aily",
...
def governance_tier(
    *,
    ...
    if honesty_score < 70 or (evidence_grade == "C" and run_support_count == 0):
        return "Low"
    if honesty_score >= 85 and maturity_score >= 75 and evidence_grade in {"A", "B"} and run_support_count >= 2 and open_question_density <= 0.35:
        return "High"
...
def routing_effect_for_object(honesty_score: float, governance_score: float) -> str:
    if honesty_score < 70:
...
def autonomy_cap_for_object(honesty_score: float, governance_score: float) -> str:
    if honesty_score < 70:
        return "suggest"
...
def writeback_trust_for_object(object_type: str, honesty_score: float, maturity_score: float) -> bool:
    if object_type not in {"skill", "agent"}:
        return False
    return honesty_score >= 85 and maturity_score >= 75
...
def governance_candidate_actions(
    *,
    ...
    worst = sorted(low_honesty, key=lambda item: item["honesty_score"])[:6]
    worst_names = [str(item["display_name"]) for item in worst]
    ...
    actions = [
        {
            ...
            "recommended_next_step": f"先处理 {worst_names[0] if worst_names else '当前最低诚实度对象'} 的 claim/evidence/boundary 三项。",
        },
        {
            "id": "B",
            "type": "workflow hardening",
...
def build_governance_review_payload(
    *,
    ...
    low_honesty = [item for item in all_rows if float(item["honesty_score"]) < 70]
    review = {
        "run_id": run_id,
        "created_at": created_at,
        "mode": mode,
...
def score_candidate(
    ...
    if name == "skill-creator" and signals["skill_creation"]:
        special_bonus = 4
    if name == "feishu-km" and signals["feishu_km"]:
        special_bonus = 5
...
def render_feishu_km_synthesis_template(request_payload: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "## Planning Boundary",
            "",
            "- 只有在 `feishu-km-record.json` 已存在后，才允许进入知识库综合和行动规划。",
            "",
        ]
    )
...
def render_capability_baseline_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Moltbook: `{payload['external_sync']['moltbook']['status']}` (`{payload['external_sync']['moltbook']['canonical_role']}`)",
            f"- Moltbook URL: `{payload['external_sync']['moltbook']['url'] or 'none'}`",
            f"- Moltbook Mode: `{payload['external_sync']['moltbook']['mode']}`",
            f"- Moltbook Probe: `{payload['external_sync']['moltbook']['probe'].get('status', 'unknown')}` / HTTP `{payload['external_sync']['moltbook']['probe'].get('http_status', 0)}`",
            "",
        ]
    )
    return "\n".join(lines)
...
def render_worklog_markdown(worklog: dict[str, Any]) -> str:
    lines = [
        ...
        "## Moltbook Mirror",
        "",
        f"- Sync Status: {worklog['moltbook_sync_status'] or 'none'}",
        f"- Human Status: {worklog['moltbook_sync_status_human'] or 'none'}",
        f"- Status Markdown: {worklog['moltbook_status_md_path'] or 'none'}",
        "",
        "## GitHub Mirror",
        "",
...
def render_evolution_markdown(evolution: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Moltbook Mirror",
            "",
            f"- Moltbook Sync Status: `{evolution.get('moltbook_sync_status') or 'none'}`",
            "- Moltbook Status Markdown: `moltbook-status.md`",
        ]
    )
...
def render_moltbook_status_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# Moltbook Status",
        "",
        f"- Task / Project: {payload['task_name']}",
        f"- Current Phase: {payload['current_phase']}",
        f"- Overall Status: {payload['overall_status']}",
        f"- Owner Skill / Agent: {payload['owner_display']}",
        f"- Completed This Round: {payload['completed_this_round']}",
        f"- Current Blockers: {payload['blockers']}",
        f"- Next Step: {payload['next_step']}",
        f"- Recent Update: {payload['recent_update']}",
        f"- Run ID: {payload['run_id']}",
        f"- Local Evidence: {payload['local_evidence_pointer']}",
    ]
    if payload.get("github_issue_url"):
        lines.append(f"- GitHub Issue: {payload['github_issue_url']}")
